...
hashiApp.controller('PostAdClassifiedComputerController', function($scope, $state, auth,categoryFactory,
		categoryRange, $stateParams, mapFactory, fileUploaderFactory, productFactory, bucketName,
		$window, $translate, categoryRange,flowFactory, $http,$timeout,conditionSomali, conditionEnglish,
		ageEnglish, ageSomali, warrantySomali, warrantyEnglish,
		computerHardDrive, computerProcessor, computerMemory, computerBrand) {
	...
	(function init() {
		...
		if($translate.use() === 'so_SO'){
			$scope.constantWarranty= warrantySomali;	 
			$scope.constantCondition=conditionSomali;
			$scope.constantAge= ageSomali;
			$scope.constantComputerBrand=computerBrand;
			$scope.constantHardDrive= computerHardDrive;
			$scope.constantComputerMemory= computerMemory;
			$scope.constantComputerProcessor= computerProcessor;
		} 
		if($translate.use() === 'en_US'){
			$scope.constantWarranty= warrantyEnglish;
		...
		/********only specific for the new product**********/
			...
			$scope.ad= {type: 'ClassifiedComputers', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
