...
hashiApp.controller('PostAdCarController', function($scope, $state, auth,categoryFactory,
		productFactory, categoryRange, $stateParams, mapFactory, fileUploaderFactory, $window, 
		coloursEnglish, coloursSomali, $translate, categoryRange, bodyTypeCarSomali, flowFactory, 
		$http, $timeout, $window, bucketName,
	...
	(function init() {
		...
		/********only specific for the new product**********/
			...
			$scope.ad= {type: 'MotorCar', productPriority: 0 , imageStatus: 'IMAGE_PENDING'};
	...
	$scope.selectedModel = function(selected) {
		if(selected){
			if (typeof selected.originalObject.modelName != "undefined") {
				$scope.ad.model=selected.originalObject.modelName;
				return;
			}else{
				$scope.ad.model=selected.originalObject;
				return;
			}
		}else{
			$scope.ad.model = null;
