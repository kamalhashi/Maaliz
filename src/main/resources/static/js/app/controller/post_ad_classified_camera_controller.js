...
hashiApp.controller('PostAdClassifiedCameraController', function($scope, $state, auth,categoryFactory,
		...
		warrantyEnglish, warrantySomali, $http,$timeout,conditionSomali, conditionEnglish,
		ageEnglish, ageSomali, cameraBrand) {
	...
	(function init() {
		...
		if($translate.use() === 'so_SO'){
			...
			$scope.constantCameraBrand= cameraBrand;	 
		} 
		...
		/********only specific for the new product**********/
			...
			$scope.ad= {type: 'ClassifiedCameras', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
