	...
	function getRootCategoriesByCategoryId(categoryId) {
		categoryFactory.getRootCategoriesByCategoryId(categoryId).success(
				function(data) {
					...
					if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_BABY_ITEMS_MIN  && 
					...
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_BOOKS_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_BOOKS_MAX)
					{
						$scope.ad= {type: 'ClassifiedBooks', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_BUSINESS_INDUSTRIAL_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_BUSINESS_INDUSTRIAL_MAX)
					{
						$scope.ad= {type: 'ClassifiedBusinessIndustrial', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_CLOTHING_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_CLOTHING_MAX)
					{
						$scope.ad= {type: 'ClassifiedClothing', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_COLLECTIBLES_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_COLLECTIBLES_MAX)
					{
						$scope.ad= {type: 'ClassifiedCollectibles', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_COMPUTER_NETWORKING_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_COMPUTER_NETWORKING_MAX)
					{
						$scope.ad= {type: 'ClassifiedNetworking', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_ELECTRONICS_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_ELECTRONICS_MAX)
					{
						$scope.ad= {type: 'ClassifiedElectronics', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_FURNITURE_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_FURNITURE_MAX)
					{
						$scope.ad= {type: 'ClassifiedFurniture', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_GAMING_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_GAMING_MAX)
					{
						$scope.ad= {type: 'ClassifiedGaming', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_HOME_APPLIANCES_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_HOME_APPLIANCES_MAX)
					{
						$scope.ad= {type: 'ClassifiedHomeAppliances', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_JEWELRY_WATCHES_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_JEWELRY_WATCHES_MAX)
					{
						$scope.ad= {type: 'ClassifiedJewelry', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_MISC_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_MISC_MAX)
					{
						$scope.ad= {type: 'ClassifiedMisc', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_MOBILE_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_MOBILE_MAX)
					{
						$scope.ad= {type: 'ClassifiedMobile', productPriority: 0, imageStatus: 'IMAGE_PENDING'};
					}
					else if($scope.categoryObject.lft >= categoryRange.CLASSIFIED_SPORTS_EQUIPMENT_MIN  && 
							$scope.categoryObject.rgt <= categoryRange.CLASSIFIED_SPORTS_EQUIPMENT_MAX)
					{
